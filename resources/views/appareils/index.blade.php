    ...
    <div class="card shadow-sm border-0">
        ...
        <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
            <h5 class="mb-0 fw-bold text-primary">
                <i class="bi-laptop me-2"></i>Liste des Appareils
            </h5>
            <form class="d-flex" role="search" action="{{ route('appareils.index') }}" method="GET">
            ...
            @if(auth()->user() && auth()->user()->role === 'admin')
                 <a href="{{route('appareils.create')}}" class="btn btn-primary btn-sm">Ajouter Appareil</a>
            @endif
        </div>
                                ...
                                <td class="ps-4">
                                    <div class="d-flex align-items-center">
                                        <div class="avatar-sm bg-light text-primary rounded-circle d-flex align-items-center justify-content-center me-3 fw-bold" style="width: 40px; height: 40px; border: 1px solid #e0e0e0;">
                                            {{ strtoupper(substr($appareil->nom_appareil, 0, 1))}}
                                        </div>
                                        <div>
                                            <div class="fw-bold text-dark">{{ $appareil->nom_appareil }}</div>
                                            <small class="text-muted">{{ $appareil->marque_appareil }}</small>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                ...
                                <td>
                                    @if(strtolower($appareil->etat_appareil) == 'reparer')
                                        ...
                                        <span class="badge rounded-pill bg-success-subtle text-success border border-success">
                                            <i class="bi bi-dash-circle-fill me-1"></i> Reparer
                                        </span>
                                    @else
                                        {{-- Etat Endommager --}}
                                        <span class="badge rounded-pill bg-danger-subtle text-danger border border-danger ">
                                            <i class="bi bi-exclamation-triangle-fill me-1"></i> Endommager
                                        </span>
                                    @endif
                                </td>
                                <td>
                                    <span class="badge bg-secondary-subtle text-secondary">
                                        {{ $appareil->couleur_appareil }}
                                    </span>
                                </td>
                                <td class="text-center">
                                    <div class="btn-group shadow-sm">
                                        <a href="{{route('appareils.show', $appareil->id_appareil)}}" class="btn btn-outline-light btn-sm text-black border" title="Voir">
                                           <i class="bi bi-eye"></i>
                                        </a>
                                        @if(auth()->user() && auth()->user()->role === 'admin')
                                            <a href="{{route('appareils.edit', $appareil->id_appareil)}}" class="btn btn-outline-light btn-sm text-black border" title="Modifier">
                                                <i class="bi bi-pencil"></i>
                                            </a>
                                                    <button type="button" class="btn btn-outline-light btn-sm text-danger border" 
                                                            data-bs-toggle="modal" 
                                                            data-bs-target="#Supprimer{{ $appareil->id_appareil }}" 
                                                            title="Supprimer">
                                                        ...
                                                        <div class="modal-dialog modal-dialog-centered">
                                                            <div class="modal-content">
                                                                ...
                                                                <div class="modal-body text-center py-4">
                                                                    ...
                                                                    <p class="mb-0">Voulez-vous vraiment supprimer l'appareil :</p>
                                                                    <h5 class="fw-bold">{{ $appareil->nom_appareil }} ({{ $appareil->marque_appareil }}) ?</h5>
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-5">
                                    <div class="text-muted">
                                        ...
                                        <p class="mt-2">Aucun appareil trouvé.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
