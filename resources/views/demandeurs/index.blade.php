    ...
    <div class="card shadow-sm border-0">
        <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
            <h5 class="mb-0 fw-bold text-primary">
                <i class="bi bi-people-fill me-2"></i>Liste des Demandeurs
            </h5>
            <form class="d-flex" role="search" action="{{ route('demandeurs.index') }}" method="GET">
                <input class="form-control me-2" 
                    name="search" 
                    type="search" 
                    placeholder="Nom, Prenom, Servic..." 
                    aria-label="Search" 
                    value="{{ request('search') }}"/>
                <button class="btn btn-outline-primary" type="submit">Recherche</button>
            </form>
            @if(auth()->user() && auth()->user()->role === 'admin')
                 <a href="{{route('demandeurs.create')}}" class="btn btn-primary btn-sm">Nouveau Demandeur</a>
            @endif
        </div>
    </div>
                            ...
                            <tr>
                                <td class="ps-4">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            <div class="fw-bold text-dark">{{ $demandeur->nom_demandeur }}</div>
                                            <small class="text-muted">{{ $demandeur->email_demandeur }}</small>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <span class="badge bg-info-subtle text-info px-2 py-1">
                                        {{ $demandeur->telephone_demandeur ?? 'N/A' }}
                                    </span>
                                </td>
                                <td>
                                    ...
                                    @else
                                        <span class="text-danger"><i class="bi bi-gender-female"></i>Feminin</span>
                                    @endif
                                </td>
                                <td>
                                ...
                                <td class="text-center">
                                    <div class="btn-group shadow-sm">
                                        <a href="{{route('demandeurs.show', $demandeur->id_utilisateur)}}" class="btn btn-outline-light btn-sm text-black border" title="Voir">
                                           <i class="bi bi-eye"></i>
                                        </a>
                                        @if(auth()->user() && auth()->user()->role === 'admin')
                                            <a href="{{route('demandeurs.edit', $demandeur->id_utilisateur)}}" class="btn btn-outline-light btn-sm text-black border" title="Modifier">
                                                <i class="bi bi-pencil"></i>
                                            </a>
                                                    <button type="button" class="btn btn-outline-light btn-sm text-danger border" 
                                                            data-bs-toggle="modal" 
                                                            data-bs-target="#Supprimer{{ $demandeur->id_utilisateur }}" 
                                                            title="Supprimer">
                                                        ...
                                                        <div class="modal-dialog modal-dialog-centered">
                                                            <div class="modal-content">
                                                                ...
                                                                <div class="modal-body text-center py-4">
                                                                    ...
                                                                    <p class="mb-0">Voulez-vous vraiment supprimer le demandeur :</p>
                                                                    <h5 class="fw-bold">{{ $demandeur->nom_demandeur }} {{ $demandeur->prenom_demandeur }} ?</h5>
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-5">
                                    <div class="text-muted">
                                        ...
                                        <p class="mt-2">Aucun demandeur trouvé.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse           
